...
void TextRenderer::Load(std::string font, unsigned int fontSize)
{
	...
	//disable byte-alignment restriction(ограничение)
...
{//activate corresponding render state
	...
	for (c = text.begin(); c != text.end(); c++) {
		...
		float ypos = y + (this->Characters['H'].Bearing.y - ch.Bearing.y) * scale; // в H bearing y касается верхней грани(имеет макс значение) как и Х и T
